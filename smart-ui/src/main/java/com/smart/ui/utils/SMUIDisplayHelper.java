...
/**
 * @date : 2019-07-15 11:40
 * @author: lichen
 * @email : 1960003945@qq.com
 * @description :
 */
public class SMUIDisplayHelper {
    /**
     * 屏幕密度,系统源码注释不推荐使用
     */
...
//    private static int[] sLandscapeRealSizeCache = null;
    ...
    public static int getScreenHeight(Context context) {
        ...
        if (SMUIDeviceHelper.isXiaomi() && xiaomiNavigationGestureEnabled(context)) {
...
//        if (SMUIDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
...
//        }
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, SMUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), SMUINotchHelper.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && SMUIDeviceHelper.isEssentialPhone()
                ...
                result -= 2 * SMUIStatusBarHelper.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (SMUIDeviceHelper.isHuawei() && !SMUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
                result -= SMUINotchHelper.getNotchSizeInHuawei(context)[1];
            }
    ...
    public static int getUsefulScreenHeight(Activity activity) {
        return getUsefulScreenHeight(activity, SMUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenHeight(View view) {
        return getUsefulScreenHeight(view.getContext(), SMUINotchHelper.hasNotch(view));
    }
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && SMUIDeviceHelper.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * SMUIStatusBarHelper.getStatusbarHeight(context);
